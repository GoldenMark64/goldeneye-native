...
| `objdeform-rwdata-regression.yml` | Linux/GCC, macOS/Clang and Windows/pinned WinLibs MinGW check out `n64decomp/007` at `c4356466796c697dfd298010b9bed261f9ed8c6a`, apply committed source patches through `0032` (not generated `0002` or independent `0033`), retain the unchanged `propobj.c` negative control, apply `0034`, then extract and compile the production `objDeform` slot expression and `modelGetNodeRwData` with a synthetic ROM-free harness. Each mode requires eight checks and zero unexpected failures. PR-controlled Python, C, workflow and patch content execute with `contents: read`; no ROM or generated assets are fetched. |
