...
| `model-slot-regression.yml` | `tools/test_model_slots.py` fetches the same pinned decomp source, headers and committed image-ID list; applies unchanged-base and repaired source/header patches separately; and compiles extracted production allocation, instantiation and release functions with stubbed memory/model initialization. Each case executes 106 lifecycle assertions. Linux GCC/Clang, macOS Clang and Windows MinGW GCC run both cases. Windows downloads the same SHA-256-pinned WinLibs archive as multi-ammo CI. |
