...
void osContGetReadData(OSContPad *pad)
{
    ...
    /* A latched crouch belongs to the mission where it was pressed. Resetting while
     * the front end is active prevents finishing a mission crouched from starting the
     * next one crouched too. geBindingsFrame() runs during the polls below, so a real
     * input held across the transition is still sampled normally. */
    if (geInFrontEnd()) { geBindingsReset(); }
