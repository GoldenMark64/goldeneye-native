...
# one of these is required and is documented at length in build_sim.sh:
...
build_port_layer() {
  ...
  # Objective-C++: gfx_metal.mm (inert under -DRAPI_GL, the file is one big #ifdef
  # RAPI_METAL, exactly symmetric with gfx_opengl.c compiling to nothing under
  # -DRAPI_METAL) plus port/src's two ObjC++ files -- ge_virtual_controller.mm (no-op stub
  # on macOS; real body only under TARGET_OS_IOS) and ge_launcher_metal.mm (the launcher's
  # standalone Metal context, referenced unconditionally from ge_launcher.cpp's RAPI_METAL
  # branch). Same coverage as build_ios.sh's loop. ARC (-fobjc-arc) manages the id<MTL...>
  # objects; only needed under metal but harmless (a no-op) under gl.
