...
if ( cd "$DECOMP" && git apply --reverse --check "$HERE/getv/patches/0033-cctv-lookpad.patch" ) 2>/dev/null; then
  echo "0033-cctv-lookpad.patch: already applied"
else
  ( cd "$DECOMP" && git apply "$HERE/getv/patches/0033-cctv-lookpad.patch" ) \
    || die "0033-cctv-lookpad.patch failed to apply"
fi
