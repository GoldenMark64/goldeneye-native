...
years expects. **`n64`** reproduces the port's earlier action layout, apart from the removed `V`
stand key — use it if you preferred the old keys.
...
Mouse and keyboard are both enabled by default. These friendly config keys are equivalent to the
raw gates shown after each comment:

```ini
mouse        = 1     # GETV_MOUSE
mouse_sens   = 100   # GETV_MOUSE_SENS
mouse_invert = 0     # GETV_MOUSE_INVERT
keyboard     = 1     # GETV_KEYBOARD
```

`mouse_sens` is a percentage from `1` through `1000`.
