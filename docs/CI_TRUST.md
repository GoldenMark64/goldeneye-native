...
| `windows-setup-package.yml` | Pull requests build and test the ROM-free setup wizard with `contents: read` but publish no download. A stable `vMAJOR.MINOR.PATCH` tag builds on a hosted Windows runner, hands the versioned ZIP and checksum to a separate `contents: write` job, and publishes one GitHub Release. `tools/fetch_deps_windows.ps1 -WizardOnly` downloads SHA-256-pinned WinLibs, SDL2, GLEW, and Dear ImGui archives; it does not fetch the decompilation or a ROM. Both paths run the tracked-game-data guard, byte-order self-test, embedded-bootstrap syntax check, import inspection, and forbidden-string scan. |
