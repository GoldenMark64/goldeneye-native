 ...
 * C or LSHIFT             -> crouch (through the dedicated helpers below)
 ...
 * directly (patch 0001) and lv.c calls gePortReloadPressed (patch 0032).
 *
...
static void gePortInputPollPortInner(int port, struct GePadState *out)
{
 ...
 gePortAndroidTouchInit();
         ...
         * conditions the rest of the mouse does; geMousePoll discards the backlog
         ...
         * wrong. */
 geWheelTick(!geMouseCaptureIdle() && geMouseEnabled());
#endif
