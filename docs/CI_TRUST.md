...
| `cctv-lookpad-regression.yml` | Linux and macOS jobs check out `n64decomp/007` at `c4356466796c697dfd298010b9bed261f9ed8c6a`, apply the top-level source patches through `0032` while excluding generated-asset patch `0002`, retain the pre-`0033` production `src/game/prop.c` as an unchanged negative control, then apply `0033-cctv-lookpad.patch`. `tools/tests/test_cctv_lookpad.py` extracts the production `setupCctv()` function and compiles it with a ROM-free synthetic CCTV harness, requiring the unchanged source to reproduce 4 failures across 6 checks and the repaired source to pass all 6. Windows performs the same regression with the existing SHA-256-pinned WinLibs toolchain. No ROM or generated assets are fetched. |
