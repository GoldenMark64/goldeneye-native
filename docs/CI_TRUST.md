...
| `public-artifact-safety.yml` | Runs `tools/check_no_game_data.py --tracked` and `tools/tests/test_agent_tools.py` via unittest discovery. Tests import `check_no_game_data`, `collect_bug_report`, and `compare_render_fingerprints`, which transitively imports `render_refs`. Also runs `tools/tests/test_macos_launcher_app.py` directly: six required packaging tests execute `getv/build_mac.sh` and `tools/make_macos_launcher_app.py` in synthetic build folders with stub compiler commands and an inert executable. Also runs `tools/tests/test_save_paths.py`, compiling the production save-path initializer with synthetic environment, stat and directory helpers; no real saves are accessed. No game data, compiler download or macOS window is needed. |
